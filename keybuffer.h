...
typedef struct
{
    ...
    const int               capacity;        // buffer size (TODO: rename to capacity)
    int                     size; // number of current keys in buffer (TODO: rename to size)
    int                     current_head;     // current head for circular access
} st_key_buffer_t;
