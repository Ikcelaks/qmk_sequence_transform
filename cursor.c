...
static st_trie_payload_t _cached_action;
...
bool cursor_advance_to_valid_output(st_cursor_t *cursor)
{
    ...
    if (cursor->sub_index < action->completion_len) {
    ...
    while (true) {
        ...
        ++cursor->index;
        st_key_buffer_advance_seq_ref_index(_buffer, &cursor->seq_ref_index);
        ...
        if (backspaces < action->completion_len) {
            ...
            for (cursor->sub_index = 0; cursor->sub_index < backspaces; ++cursor->sub_index) {
                const int completion_char_index = action->completion_index + action->completion_len - 1 - cursor->sub_index;
                const uint8_t triecode = CDATA(_trie, completion_char_index);
...
bool st_cursor_configure(const st_trie_t * const trie, st_key_buffer_t * const buffer)
{
    ...
    {
        _trie = trie;
        st_key_buffer_reset(_buffer);
    }
    return _trie && _buffer;
}
//////////////////////////////////////////////////////////////////
...
uint8_t st_cursor_get_triecode(st_cursor_t *cursor)
{
    ...
    const st_trie_payload_t *action = st_cursor_get_action(cursor);
    const int completion_char_index_delta = action->completion_len - 1 - cursor->sub_index;
    ...
    st_assert(completion_char_index >= 0, "Invalid completion_char_index: %d at Cursor Pos: %d, %d; %d",
                completion_char_index, cursor->index, cursor->sub_index, _buffer->size);
    const uint8_t triecode = CDATA(_trie, completion_char_index);
    if (st_is_trans_seq_ref_triecode(triecode)) {
        return st_key_buffer_get_seq_ref(_buffer, cursor->seq_ref_index);
    }
...
{
    st_trie_payload_t *action = &_cached_action;
    if (_cache_valid == cursor->index) {
        return action;
    }
    const st_key_action_t *keyaction = st_key_buffer_get(_buffer, cursor->index);
    if (!keyaction) {
    ...
    if (keyaction->action_taken == ST_DEFAULT_KEY_ACTION) {
        ...
        st_get_payload_from_match_index(_trie, action, keyaction->action_taken);
    }
    _cache_valid = cursor->index;
    return action;
}
//////////////////////////////////////////////////////////////////
...
{
    return cursor->index >= _buffer->size || cursor->seq_ref_index >= _buffer->seq_ref_capacity;
}
//////////////////////////////////////////////////////////////////
...
bool st_cursor_next(st_cursor_t *cursor)
{
    ...
    ++cursor->sub_index;
    const st_trie_payload_t *action = st_cursor_get_action(cursor);
    if (action->completion_len > cursor->sub_index) {
        const int completion_char_index = action->completion_index + action->completion_len - 1 - cursor->sub_index;
        const uint8_t triecode = CDATA(_trie, completion_char_index);
        if (st_is_trans_seq_ref_triecode(triecode)) {
            // This is a seq_ref, increment the seq_ref_index
...
{
    const int cur_pos = (cursor->index << 8)
        + cursor->sub_index;
...
{
#ifndef NO_PRINT
    st_cursor_t cursor_pos = st_cursor_save(cursor);
    uprintf("cursor: |");
